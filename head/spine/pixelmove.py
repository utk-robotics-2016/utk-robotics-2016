from head.spine import optical_move
...
def x_strafe(s, actual_x, ideal_x, scale):
    ...
    diff *= scale
    # We don't actually use this to move but we need to keep track
    strafe_time = 0.0537 + 0.01698 * abs(diff)
    optical_ticks = int(max(-127.3 + 43.7 * abs(diff), 0))
    mult = 1
    if diff < 0:  # to right
        optical_move.x_move(s, optical_ticks, 'right')
        mult = 1
    elif diff > 0:  # to left
        optical_move.x_move(s, optical_ticks, 'left')
        mult = -1
